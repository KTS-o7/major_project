...
def train_model(model_name, dataset_path, max_length, output_dir, num_epochs, batch_size):
    logger.info(f"Starting training for model: {model_name} on MPS")
    if not torch.backends.mps.is_available():
        raise RuntimeError("MPS not available. Please check your PyTorch installation.")
    ...
    training_args = TrainingArguments(
        ...
        eval_steps=10,
        save_steps=50,
        fp16=False,  # MPS doesn't support mixed precision
        optim="adamw_torch",
...
def main():
    parser = argparse.ArgumentParser(description="Finetune a Causal LM on Apple Silicon with LoRA")
