...
def download_2k_dataset(dataset_name, output_dir):
    ...
    if not os.path.exists(download_path):
        logger.warning(f"Downloading {dataset_name} 2k dataset...")
        download_file(url, download_path)
    else:
        logger.info(f"{dataset_name} 2k dataset already downloaded to {download_path}")
...
def main():
    ...
    if args.list:
        logger.info("Available LogHub 2k datasets:")
    ...
    for dataset_name in args.datasets:
        try:
            ...
            logger.info(f"Successfully downloaded {dataset_name} 2k dataset to {path}")
        except Exception as e:
            logger.error(f"Error downloading {dataset_name} 2k dataset: {str(e)}")
